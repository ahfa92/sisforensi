...
class eduperiod_m extends CI_Model {


	protected $table = 'educational_period';
	protected $primary_key = 'edu_period_id';

	public function list($cond = null)
	{
		$this->db
			->join('educational_year e','e.edu_year_id = '.$this->table.'.edu_year_id','left')
	...
	public function date_period($date)
	{

		$this->db
			->where('edu_period_start <=',$date)
			->where('edu_period_end >=',$date)
			->join('educational_year e','e.edu_year_id = '.$this->table.'.edu_year_id','left')
			->where($this->table.'.deleted_at is null');
