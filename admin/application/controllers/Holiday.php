	...
	public function index()
	{
		if($this->input->post('action') == 'delete'){
			if($this->holiday_m->delete()){
				...
				$this->load->view('holiday_v',array('page' => $page,'holidays' => $holidays));
			}
		} else if($this->input->post('action') == 'new'){
			$holiday = new stdClass();
			$page['title'] = 'Edit Holiday';

			$this->load->view('holiday_form_v',array('page' => $page,'holiday' => $holiday));
			...
			if($holiday->num_rows() == 1){
				...
				$this->load->view('holiday_form_v',array('page' => $page,'holiday' => $holiday->row()));
			} else {
