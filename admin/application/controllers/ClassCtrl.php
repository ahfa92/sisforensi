	...
	public function index()
	{
		if($this->input->post('action') == 'delete'){
			if($this->class_m->delete()){
				...
				$this->load->view('class_v',array('page' => $page,'classes' => $classes));
			}
		} else if($this->input->post('action') == 'new'){
			$class = new stdClass();
			$page['title'] = 'Edit Kelas';
			$this->load->view('class_form_v',array('page' => $page,'class' => $class));
			...
			if($class->num_rows() == 1){
				$page['title'] = 'Edit Kelas';
				$this->load->view('class_form_v',array('page' => $page,'class' => $class->row()));
			} else {
